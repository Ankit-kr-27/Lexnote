...
const TopicForm = ({ setResult, setLoading, loading, setError }) => {
  ...
  return (
    <motion.div
      ...
      className='rounded-2xl bg-gradient-to-br from-black/90 via-black/80 to-black/90 backdrop-blur-2xl border border-white/10 shadow-[0_20px_45px_rgba(0,0,0,0.75)] p-8 space-y-6 text-white'>
      ...
      <motion.button
        ...
        disabled={loading}
        className={`w-full mt-4 py-3 rounded-xl font-semibold flex items-center justify-center gap-3 transition ${loading ? "bg-gray-300 text-gray-600 cursor-not-allowed" : "bg-gradient-to-br from-white to-gray-200 text-black shadow-[0_15px_35px_rgba(0,0,0,0.6)]"
          }`}>
      ...
      <div className='mt-4 space-y-2'>
        ...
        <p className='text-sm text-gray-400 text-center'>Please wait while we generate your notes...</p>
...
function Toggle({ label, checked, onChange }) {
  return (
    <div className='flex items-center gap-4 cursor-pointer select-none' onClick={onChange}>
      ...
      <span className={`text-sm transition-colors ${checked ? "text-green-300" : "text-white"}`}>{label}</span>
